...
function getQualityLabel(
  info: ReleaseInfo["mediaInfo"]["video"]["quality"],
): string {
...
function getCodecLabel(
  codec: ReleaseInfo["mediaInfo"]["video"]["codec"],
): string {
...
export function formatTitle(
  ...
  if (info.type === "show") {
    if (info.episode === null) {
      ...
      parts.push(
        "E" + String(sorted[0]).padStart(2, "0") + "-E" +
          String(sorted[sorted.length - 1]).padStart(2, "0"),
      );
    } else {
      parts.push("S" + String(info.season).padStart(2, "0"));
